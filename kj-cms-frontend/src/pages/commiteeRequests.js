...
import { CommiteeAPI, getAllProjectsAPI, getFacultyAPI, getStudentAPI } from 'src/config/api';
...
import { CommiteeRequestTable } from 'src/sections/commiteeRequests/CommiteeRequestTable';
...
const Page = () => {
    ...
    async function _getData() {
        try {
            let res = await axios.get(CommiteeAPI + `?status=${'pending'}`)
    ...
    return (
        <>
            ...
            <CommiteeAdd open={openAddModal} setOpen={setOpenAddModal} getData={_getData} />
        </>
    );
};
